...
public class SenhaFrame extends JFrame 
{
	...
	class BotaoAlterarListener implements ActionListener 
	{
		...
		{	
			...
			{
				CadastraUsuarioPerfilFrame.setRotinaExecutada(2);
				new CadastraUsuarioPerfilFrame();
				dispose();
			}
			else
			{
		...
		class BotaoIncluirListener implements ActionListener 
		{
			...
			{	
				...
				{
					CadastraUsuarioPerfilFrame.setRotinaExecutada(1);
					new CadastraUsuarioPerfilFrame();
					dispose();
				}
				else
				...
				{	//PEGAMOS O CAMINHO ATRAVÈS DO GETSELECTES ROW = RETORNA INDICE
					...
					if(result != 0 )
					{
						...
						validar.removeUsuario();
				   		/*ATUALIZANDO A TABELA DE USUARIOS DEPOIS DA REMOÇÂO*/
	               		refreshTabela();
						textLogin.setText("");
					}
					else
